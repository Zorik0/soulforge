...
const MasterKeyPage = () => {
  ...
  const handleProceed = (currentStepNum) => {
    ...
    const redirectUrl = `${LINKVERTISE_LINKS[currentStepNum]}?r=${btoa(
      window.location.href.split("?")[0] + `?c=${nextStepParam}`
    )}`;
...
const HeroLoadstring = ({ setPage }) => {
  const [copyStatus, setCopyStatus] = useState("idle");
  const loadstring =
    'loadstring(game:HttpGet("https://scriptforge.gg/main"))()';
